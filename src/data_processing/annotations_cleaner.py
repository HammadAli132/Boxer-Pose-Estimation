...
def final_cleanup(ann_dir, video_name):
    """
    Perform final cleanup on annotations:
    1. Convert keypoints from current format to target format
    2. Replace categories with predefined structure
    3. Change category_id 16 to 1 and 1 to 2 in all annotations
    4. Fix frame names
    
    Current format: [nose, neck, left_shoulder, right_shoulder, left_hip, right_hip, 
                     left_elbow, left_wrist, right_elbow, right_wrist, left_knee, 
                     left_ankle, right_knee, right_ankle]
    
    Target format: [left_shoulder, right_shoulder, left_hip, right_hip, left_knee, 
                    right_knee, left_ankle, right_ankle, left_elbow, left_wrist, 
                    right_elbow, right_wrist, neck, nose]
    """
    annotations_path = ann_dir / "annotations.json"
    if not annotations_path.exists():
        print(f"❌ No annotations.json found in {ann_dir}")
    ...
    # Current: [nose, neck, left_shoulder, right_shoulder, left_hip, right_hip, 
    #           left_elbow, left_wrist, right_elbow, right_wrist, left_knee, 
    #           left_ankle, right_knee, right_ankle]
    # Target: [left_shoulder, right_shoulder, left_hip, right_hip, left_knee, 
    #          right_knee, left_ankle, right_ankle, left_elbow, left_wrist, 
    #          right_elbow, right_wrist, neck, nose]
    ...
    print(f"✅ Converted keypoints for {len(data['annotations'])} annotations")
    ...
    # Step 4: Fix frame names
    print(f"\n🔄 Step 3: Fixing frame names...")
    fix_frame_names(ann_dir, video_name)
    
    # Verify the cleanup
    print(f"\n🔍 Final Verification:")
    verify_final_cleanup(data)
    
    print(f"\n✅ Final cleanup complete!")
    return True


def verify_final_cleanup(data):
    """Verify that the final cleanup was successful"""
    
    # Check categories
    print(f"\nCategories ({len(data['categories'])}):")
    for cat in data['categories']:
        print(f"  - {cat['name']} (id={cat['id']})")
        print(f"    Keypoints: {len(cat['keypoints'])}")
    ...
    # Check for category_id 16
    if 16 in category_counts:
        print(f"\n⚠️  Warning: Found {category_counts[16]} annotations still using category_id=16!")
    else:
        print(f"\n✅ No annotations with category_id=16 found")
    
    # Check keypoint counts
    if data['annotations']:
        first_ann = data['annotations'][0]
        print(f"\nKeypoint validation:")
        print(f"  - Expected keypoints per annotation: 42 values (14 keypoints × 3)")
        print(f"  - Actual in first annotation: {len(first_ann['keypoints'])} values")
    ...
    if not unexpected:
        print(f"✅ All annotations use valid category IDs (1 or 2)")
    else:
        print(f"❌ Found unexpected category IDs: {unexpected}")
...
def show_menu():
    ...
    print("7. Final Cleanup (keypoints, categories, frame names)")
    print("0. Exit")
    print("\n" + "="*60)
    ...
    while True:
        ...
        try:
            ...
            if choice == "0":
                ...
                # Final Cleanup
                result = list_annotation_dirs("Select directory for final cleanup:")
                if result:
                    ann_dir, video_name = result
                    final_cleanup(ann_dir, video_name)
