...
PROCESSED_TEST_IMAGES = PROCESSED_IMAGES_DIR / "test"
...
def save_split(ann_data, images, annotations, split_name, source_frames_dir=None, hinted_video=None, skip_annotations=False, copy_images=False):
    """
    ...
    for img in tqdm(images, desc=f"Processing -> {split_name}", unit="img"):
        ...
        if src_path and src_path.exists():
            ...
            # Copy image if requested (for test split)
            if copy_images:
                dest_dir = PROCESSED_TEST_IMAGES if split_name == "test" else PROCESSED_IMAGES_DIR / split_name
...
def create_test_split_from_video(video_name, start_frame, frame_count):
    ...
    # Clean existing test images directory
    if PROCESSED_TEST_IMAGES.exists():
        import shutil
        shutil.rmtree(PROCESSED_TEST_IMAGES)
        print(f"🧹 Cleaned existing test images directory") 
    ...
    return save_split(ann_data, test_images, [], "test", hinted_video=video_name, skip_annotations=False, copy_images=True)
